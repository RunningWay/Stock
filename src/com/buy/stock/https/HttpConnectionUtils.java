...
public class HttpConnectionUtils {
	...
	public static HttpResponseEntity postByHttpUrlConnection(String pathUrl,
		...
		try {
			...
			hre.setContent(readInputStream(input));
			return hre;
		} catch (Exception ex) {
	...
	// 上传，第一个参数，为要使用的URL，第二个参数，为表单内容，第三个参数为要上传的文件，可以上传多个文件，这根据需要页定
	public static HttpResponseEntity put(String actionUrl, String token,
			Bitmap bm) {
		...
		try {
			...
			sb1.append(BOUNDARY);
			sb1.append(LINEND);
			sb1.append("Content-Disposition: form-data; name=\"face\"; filename=\"head.jpg\""
					+ LINEND);
	...
	public static HttpResponseEntity putByHttpUrlConnection(String pathUrl,
			String requestString) {
		HttpResponseEntity hre = new HttpResponseEntity();
		InputStream input = null;
		HttpURLConnection httpConn = null;
